...
export default function ReviewUser() {
  ...
  if (!user) {
    return (
      <div className="max-w-4xl mx-auto py-8">
        <p className="text-muted-foreground">Unable to load application details.</p>
      </div>
    );
  }
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-3">
          <Button
            ...
            size="sm"
            className="gap-2"
            onClick={() => router.push("/sangha/pending-users")}
          >
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Verify the details before approving or rejecting this application
            </p>
          </div>
        </div>
        ...
        <CardContent>
          <dl className="grid md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <dd className="font-medium">{user.village || "-"}</dd>
            </div>
            <div>
              <dt className="text-sm text-muted-foreground">Phone</dt>
              <dd className="font-medium">{user.phone || personal.phone || "-"}</dd>
            </div>
            <div>
              <dt className="text-sm text-muted-foreground">Email</dt>
              <dd className="font-medium">{user.email || personal.email || "-"}</dd>
            </div>
          </dl>
        </CardContent>
      </Card>
        ...
        <Card className="shadow-sm">
          ...
          <CardContent>
            {Object.keys(occupation).length === 0 ? (
              <p className="text-sm text-muted-foreground">No occupation details available.</p>
            ) : (
      ...
      <Card className="shadow-sm bg-secondary/30">
        <CardHeader>
          ...
          <CardDescription>
            Approve if the details are correct and complete, otherwise reject with corrections.
          </CardDescription>
        </CardHeader>
