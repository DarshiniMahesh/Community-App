...
export default function ApprovalsPage() {
  ...
  // Note: pendingSanghas is now empty because PENDING_SANGHA is empty in mockData
  ...
  const displayUsers = viewStatus === 'pending' 
    ? pendingUsers 
    : USERS.filter(u => u.status === viewStatus);

  const displaySanghas = viewStatus === 'pending' 
    ? pendingSanghas 
    : SANGHA_LIST.filter(s => s.status === viewStatus);
  ...
  return (
    <div className="page">
      ...
      <div className="radio-tab-group">
        {tabs.map(t => (
          <label key={t.id} className={`radio-tab ${tab === t.id ? 'active' : ''}`} onClick={() => { setTab(t.id); setViewStatus('pending'); }}>
      ...
      {tab === 'user' && (
        <>
          <div style={{ display: 'flex', gap: '8px', marginBottom: '20px', marginTop: '10px' }}>
            <button onClick={() => setViewStatus('pending')} className={`btn ${viewStatus === 'pending' ? 'btn-primary' : 'btn-secondary'} btn-sm`}>Pending</button>
            <button onClick={() => setViewStatus('approved')} className={`btn ${viewStatus === 'approved' ? 'btn-success' : 'btn-secondary'} btn-sm`}>Approved</button>
            <button onClick={() => setViewStatus('rejected')} className={`btn ${viewStatus === 'rejected' ? 'btn-danger' : 'btn-secondary'} btn-sm`}>Rejected</button>
          </div>

          {displayUsers.length === 0 ? (
            <div className="card" style={{ textAlign: 'center', padding: 40, color: 'var(--gray-400)' }}>No {viewStatus} users found.</div>
          ) : (
            displayUsers.map(u => (
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14, flex: 1 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      {viewStatus === 'approved' && (
                        <div style={{ marginTop: 8, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '8px 16px', background: 'var(--gray-50)', padding: '8px 12px', borderRadius: 6 }}>
                          <div><div style={{ fontSize: 10, color: 'var(--gray-500)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Approved Date</div><div style={{ fontSize: 13, fontWeight: 600, color: 'var(--gray-800)' }}>{u.approvedDate || '—'}</div></div>
                          <div><div style={{ fontSize: 10, color: 'var(--gray-500)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Approved By</div><div style={{ fontSize: 13, fontWeight: 600, color: 'var(--gray-800)' }}>{u.approvedBy || '—'}</div></div>
                        </div>
                      )}

                      {viewStatus === 'rejected' && (
                         <div style={{ marginTop: 8, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '8px 16px', background: '#fef2f2', padding: '8px 12px', borderRadius: 6 }}>
                         <div><div style={{ fontSize: 10, color: 'var(--gray-500)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Rejected Date</div><div style={{ fontSize: 13, fontWeight: 600, color: '#991b1b' }}>{u.rejectedDate || '—'}</div></div>
                         <div><div style={{ fontSize: 10, color: 'var(--gray-500)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Rejected By</div><div style={{ fontSize: 13, fontWeight: 600, color: '#991b1b' }}>{u.rejectedBy || '—'}</div></div>
                       </div>
                      )}
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    {viewStatus === 'pending' ? (
                      ...
                      <button className="btn btn-secondary btn-sm" onClick={() => openView(u)}>View Details</button>
                    )}
                  </div>
                </div>
              </div>
      ...
      {tab === 'sangha' && (
        <>
          ...
          {displaySanghas.length === 0 ? (
             <div className="card" style={{ textAlign: 'center', padding: 40, color: 'var(--gray-400)' }}>No {viewStatus} sanghas found.</div>
          ) : (
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14, flex: 1 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      {viewStatus === 'approved' && (
                        <div style={{ marginTop: 8, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '8px 16px', background: 'var(--gray-50)', padding: '8px 12px', borderRadius: 6 }}>
                          <div><div style={{ fontSize: 10, color: 'var(--gray-500)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Approved Date</div><div style={{ fontSize: 13, fontWeight: 600, color: 'var(--gray-800)' }}>{s.approvedDate || '—'}</div></div>
                          <div><div style={{ fontSize: 10, color: 'var(--gray-500)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Approved By</div><div style={{ fontSize: 13, fontWeight: 600, color: 'var(--gray-800)' }}>{s.approvedBy || '—'}</div></div>
                        </div>
                      )}

                      {/* Show Rejected Details - Displays the date correctly here */}
                      {viewStatus === 'rejected' && (
                        <div style={{ marginTop: 8, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '8px 16px', background: '#fef2f2', padding: '8px 12px', borderRadius: 6 }}>
                          <div><div style={{ fontSize: 10, color: 'var(--gray-500)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Rejected Date</div><div style={{ fontSize: 13, fontWeight: 600, color: '#991b1b' }}>{s.rejectedDate || '—'}</div></div>
                          <div><div style={{ fontSize: 10, color: 'var(--gray-500)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Rejected By</div><div style={{ fontSize: 13, fontWeight: 600, color: '#991b1b' }}>{s.rejectedBy || '—'}</div></div>
                        </div>
                      )}
